...
Jutils is a visualization toolkit for alternative splicing events. Jutils directly supports visualizing results generated by the differential splicing (DS) detection tools MntJULiP, LeafCutter, MAJIQ and rMATS, and can be easily adapted to use with other DS software.
...
This branch contains the working version of Jutils, which additionally implements PCA visualizations. For the original (stable) version of Jutils, please refer to the [master](https://github.com/splicebox/Jutils/tree/master) branch.
```
