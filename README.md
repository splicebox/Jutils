...
User may want to visualize the differential splicing abundance (DSA) resutls generated by MntJULiP, they can apply option `--avg` to filter out introns with low estimated read counts and `--fold-change` to select the significant changed events.
